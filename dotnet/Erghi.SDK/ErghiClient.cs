...
/// </summary>
public sealed class ErghiClient : IAsyncDisposable
{
    ...
    public ErghiClient(ErghiConfig config)
    {
    ...
    /// </summary>
    public async Task<string> AuthenticateVisitorAsync(string widgetId, string jwtToken, CancellationToken ct = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new AuthenticationException($"Failed to authenticate visitor: {err}");
        }

        var data = await response.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>(cancellationToken: ct);
        if (data.TryGetProperty("visitorId", out var vId) || data.TryGetProperty("VisitorId", out vId))
        {
